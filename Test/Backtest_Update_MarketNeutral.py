...
#Value_Growth=['PE_IndustryRank_Diff_OPP_90']
...
def Prepare4Score(Dict,FactorPath,IndexComponentPath):
    ...
    for Date in DateList:
        ...
        df=pd.DataFrame()
        dfIndexComponent=IndexComponent[IndexComponent['date']==Date]
        Code=dfIndexComponent['code'].values.tolist()        
        df['Code']=Code
        df['Index']=dfIndexComponent['index'].values.tolist() 
        for Factor in FactorList:
...
#输出信号
def Output_Signal(Code,Industry,IndexList,SignalPath):
    df=pd.DataFrame()
    df['Code']=Code
...
def Classify_Signal(df,Amount,Flag,OutputPath):        
    df=df[df['Score']!=0]
    df=df[df['Industry']!=u'其他']
    IndexComponent=list(set(df['Index'].values.tolist()))
    StockNum=[float(filter(str.isdigit, a)) for a in IndexComponent]
    StockPercent=[a/sum(StockNum) for a in StockNum]
    Code=[]
    Industry=[]
    IndexList=[]
    for Loc in range(len(IndexComponent)):
        dfIndex=df[df['Index']==IndexComponent[Loc]]
        Percent=Amount*StockPercent[Loc]/len(dfIndex)
        if Flag=='Top':            
            Signal=dfIndex.groupby(by=['Industry']).apply(lambda x: x.head(int(round(len(x)*Percent,0))))
        else:
            Signal=dfIndex.groupby(by=['Industry']).apply(lambda x: x.tail(int(round(len(x)*Percent,0))))
            
        Code=Code+Signal.index.get_level_values(1).tolist()
        Industry=Industry+Signal['Industry'].values.tolist()
        IndexList=IndexList+Signal['Index'].values.tolist()
        Output_Signal(Code,Industry,IndexList,OutputPath)
...
def Get_Signal(StockPath,FactorPath,ScorePath,TopSignalPath,BottomSignalPath,Period,Amount):
    ...
    for i in range(0,len(FileList)-1,Period):
        ...
        Score=Data.groupby(by=['Industry']).rank().fillna(value=0)
        del Score['Index']
        Score=Score.T.sum()
        df=pd.DataFrame()
        df['Score']=Score
        df['Industry']=Data['Industry'].values.tolist()     
        df['Index']=Data['Index'].values.tolist()
        df=df.sort_values(by=['Industry','Index','Score'],ascending=False)
        df.to_csv(ScorePath+File,encoding='gbk')
        Classify_Signal(df,Amount,'Top',TopSignalPath+OutputFile)
        Classify_Signal(df,Amount,'Bottom',BottomSignalPath+OutputFile)
