...
#提取所有的因子值
def Prepare4Score(Dict,FactorPath,Stock,IndexComponentPath):
    ...
    for Date in DateList[:1]:
        ...
        df['Code']=Code
        for Factor in FactorList[2:3]:
            DateIndex=CalendarDate.index(Date)
            ICPeriod=CalendarDate[DateIndex-30:DateIndex+1]

            FactorData=Dict[Factor][ICPeriod].loc[Code].T
            FactorData.index=pd.to_datetime(FactorData.index).tz_localize('UTC')
            FactorData=FactorData.stack()
            FactorData.index=FactorData.index.set_names(['Date',Factor])    
            
            AdjCloseData=AdjClose.loc[:Date]
            
            factor_data=alphalens.utils.get_clean_factor_and_forward_returns(FactorData,AdjCloseData,periods=(1,Period))
...
#
#Prepare4Score(Get_Factor_Data(FactorLibPath,FactorList),FactorPath,StockPath,IndexComponentPath)
#
